...
void Bank::removeaccount(int acc_index){
    ...
    void Bank::bank_rd_start(int thread_id){
        ...
        logfile << "thread number " << thread_id << " catch bank lock, start reading " << endl;
        ...
        while(curr_writing || wr_wait > 0){ //the logic rd_count != 0 is not included to prevent writer starvation- if there is a writer waiting then stop entering reader
            ...
            pthread_mutex_lock(&log_wrt_lck);
            logfile << "thread number " << thread_id << " is about to wait and unlock bank lock, start reading" << endl;
            pthread_mutex_unlock(&log_wrt_lck);
    ...
    void Bank::bank_rd_end(int thread_id){
        ...
        pthread_mutex_lock(&log_wrt_lck);
        logfile << "thread number " << thread_id << " catch bank lock, end reading " << endl;
        pthread_mutex_unlock(&log_wrt_lck);
        ...
        pthread_mutex_lock(&log_wrt_lck);
        logfile << "thread number " << thread_id << " is about to unlock bank lock, end reading" << endl;
        pthread_mutex_unlock(&log_wrt_lck);
    ...
    void Bank::bank_wr_start(int thread_id){
        ...
        while(rd_count > 0 || curr_writing){
            pthread_mutex_lock(&log_wrt_lck);
            logfile << "thread number " << thread_id << " is about to wait and unlock bank lock, start writing" << endl;
            pthread_mutex_unlock(&log_wrt_lck);
    ...
    void Bank::bank_wr_end(int thread_id){
        ...
        pthread_mutex_lock(&log_wrt_lck);
        logfile << "thread number " << thread_id << " is about to unlock bank lock, end writing" << endl;
        pthread_mutex_unlock(&log_wrt_lck);
        pthread_mutex_unlock(&bank_lock);
    }
...
void* printAccounts(void* args){
    ...
    while(1)
    {
        ...
        bank.bank_rd_start(print_thread_num);
        printf("\033[2J");
        printf("\033[1;1H");
        ...
        bank.bank_rd_end(print_thread_num);
        usleep(500000);
    }
}